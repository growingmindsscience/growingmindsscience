...
The shared certification layer for every compiled-content tool on the site.
One invariant everywhere (the portfolio plan's cert-harness rule): no artifact
family ships without a deterministic grader that passes the approved artifacts
and catches every planted violation, in CI on every push.
...
- **Activity Library** (`tools/activity-library.html`) — the free sampler
  (`artifacts/activities/`), graded for safety lexicon, mechanism-named
  intentions, and the deterministic Today's 3 contract
  (`graders/activities.mjs`).
- **Claims Library** (`claims/`, generated by `scripts/build-claims.mjs`) —
  evidence-graded claims (`artifacts/claims/`), graded for sourcing, honesty
  rules (no overclaim above grade, hedging on limited evidence), and page
  drift (`graders/claims.mjs` + the build script's `--check`).
- **Reading Prompt Cards** (`tools/reading-prompt-cards.html`) — dialogic
  reading deck (`artifacts/drc/`), graded for CROWD type-correctness and the
  no-book-text IP rule (`graders/drc.mjs`).
